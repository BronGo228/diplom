...
import { FileMinus, Search, Plus, Edit, Trash2, CheckCircle, Save, X, Printer, Check, Eye } from 'lucide-react';
...
export default function WriteOffs() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('http://localhost:8000/write-offs/');
      setWriteOffs(res.data);
    } catch (err) {
      console.error(err);
      alert("Ошибка при загрузке списаний: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateNew = () => {
    ...
    setEditingDoc({
      number: `OUT-${dateStr}-${randId}`,
      warehouse_id: defaultLocation,
      project_id: null,
      expense_item: 'Списания',
      comment: '',
      status: 'Draft',
      user_id: user.id
    });
  ...
  const handleEdit = async (docId) => {
    console.log("Edit clicked for doc:", docId);
    try {
      const res = await axios.get(`http://localhost:8000/write-offs/${docId}`);
      if (!res.data) throw new Error("Blank response from backend");
      setEditingDoc(res.data);
      setItems(res.data.items || []);
    } catch(err) {
      console.error("Error in handleEdit:", err);
      alert("Ошибка при открытии документа: " + err.message);
    }
  };
  ...
  const handleAddItem = () => {
    ...
    setItems([...items, {
      ...
      quantity: 1,
      price: 0,
      reason: 'Недостача (инвентаризация)'
    }]);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post('http://localhost:8000/write-offs/', payload);
      setEditingDoc(null);
      fetchData();
    } catch (err) {
      console.error(err);
      alert("Ошибка при сохранении списания");
    }
  };

  const handlePost = async (id) => {
    console.log("Post clicked for doc:", id);
    if(!window.confirm("Провести документ списания? Остатки на складе будут уменьшены и действие необратимо.")) return;
    try {
      await axios.post(`http://localhost:8000/write-offs/${id}/post?user_id=${user.id}`);
      fetchData();
    } catch(err) {
      console.error(err);
      if (err.response && err.response.data && err.response.data.detail) {
          ...
          alert("Ошибка при проведении документа списания");
      }
    }
  };
    ...
    return (
      <div style={{ padding: '32px', animation: 'fadeIn 0.3s' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontSize: '1.8rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              {editingDoc.id ? `Списание №${editingDoc.number}` : 'Новое списание'}
            </h1>
        ...
        <div className="glass-panel" style={{ padding: '24px', marginBottom: '24px', display: 'flex', gap: '40px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Склад (Источник списания)</label>
              <select style={getStyle(isPosted)} value={editingDoc.warehouse_id || ''} onChange={e => setEditingDoc({...editingDoc, warehouse_id: parseInt(e.target.value)})} disabled={isPosted}>
                {locations.length > 0 ? locations.map(l => <option key={l.id} value={l.id} style={{background: '#1a1d24'}}>{l.zone} {l.row ? `(Ряд: ${l.row})` : ''}</option>) : <option value="">Нет складов</option>}
              </select>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Статья расходов</label>
              <input type="text" style={getStyle(isPosted)} value={editingDoc.expense_item || ''} onChange={e => setEditingDoc({...editingDoc, expense_item: e.target.value})} disabled={isPosted} />
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '24px', marginBottom: '24px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
             ...
             <textarea style={getStyle(isPosted, { resize: 'vertical' })} rows={2} value={editingDoc.comment || ''} onChange={e => setEditingDoc({...editingDoc, comment: e.target.value})} disabled={isPosted} placeholder="Дополнительная информация к акту списания" />
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'rgba(0,0,0,0.02)' }}>
                ...
                <th style={{ padding: '12px 24px', textAlign: 'center', color: 'var(--text-secondary)', width: '100px' }}>Остаток</th>
                <th style={{ padding: '12px 24px', textAlign: 'left', color: 'var(--text-secondary)', width: '120px' }}>Кол-во</th>
                <th style={{ padding: '12px 24px', textAlign: 'left', color: 'var(--text-secondary)', width: '140px' }}>Цена</th>
                <th style={{ padding: '12px 24px', textAlign: 'left', color: 'var(--text-secondary)' }}>Сумма</th>
                <th style={{ padding: '12px 24px', textAlign: 'left', color: 'var(--text-secondary)' }}>Причина списания</th>
            ...
            <tbody>
              {items.map((item, idx) => {
                ...
                return (
                  <tr key={idx} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '16px 24px' }}>
                      <input type="text" style={getStyle(isPosted)} value={item.reason} onChange={e => updateItem(idx, 'reason', e.target.value)} disabled={isPosted} placeholder="Например: Недостача" />
                    </td>
                    {!isPosted && (
              ...
              {items.length === 0 && (
                <tr><td colSpan={7} style={{ padding: '32px', textAlign: 'center', color: 'var(--text-secondary)' }}>Нет позиций</td></tr>
              )}
            </tbody>
          </table>
  ...
  // LIST VIEW
  const filtered = writeOffs.filter(r => r.number.toLowerCase().includes(searchTerm.toLowerCase()) || (r.comment && r.comment.toLowerCase().includes(searchTerm.toLowerCase())));

  return (
    <div style={{ padding: '32px', animation: 'fadeIn 0.5s ease-out', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        <div style={{ background: 'linear-gradient(135deg, #6366f1, #4338ca)', color: 'white', padding: '12px', borderRadius: '16px', boxShadow: '0 8px 16px rgba(99, 102, 241, 0.2)' }}>
           <FileMinus size={28} />
        </div>
        <div style={{ flex: 1 }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 700, margin: 0, color: 'var(--text)' }}>Списания</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', margin: '4px 0 0 0' }}>Оформление недостачи и списание товаров со склада</p>
        </div>
      ...
      <div className="glass-panel" style={{ overflow: 'hidden', padding: 0 }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.2s', background: r.status === 'Draft' ? 'rgba(244,63,94,0.02)' : 'transparent', position: 'relative', zIndex: 10 }}>
                  <td style={{ padding: '16px 24px' }}>
                  ...
                  <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', position: 'relative', zIndex: 20 }}>
                       {r.status === 'Draft' && (
                         <button className="btn-secondary" onClick={() => handlePost(r.id)} style={{ color: '#10b981', padding: '6px', cursor: 'pointer' }} title="Провести документ">
                           <Check size={18} />
                         </button>
                       )}
                       <button className="btn-secondary" onClick={() => handleEdit(r.id)} style={{ padding: '6px', cursor: 'pointer' }} title={r.status === 'Posted' ? 'Просмотр' : 'Редактировать'}>
                         {r.status === 'Posted' ? <Eye size={18} /> : <Edit size={18} />}
                       </button>
                    </div>
                  </td>
                </tr>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={4} style={{ padding: 0 }}><EmptyState title="Документы не найдены" /></td></tr>
              )}
            </tbody>
          </table>
        )}
